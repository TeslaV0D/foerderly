// src/app/components/ResultCard.js
// Fix 2: Gleiche Höhe mit Flexbox, "Details ansehen" immer unten
'use client';
...
function parseDescription(text) {
  ...
  for (const part of parts) {
    const cleaned = part
      ...
      .trim();
    if (cleaned && cleaned.length > 2) tags.push(cleaned);
  }
...
export default function ResultCard({ programm, index, onClick }) {
  ...
  return (
    <button
      ...
      className="card animate-fade-up p-4 sm:p-5 text-left w-full flex flex-col h-full"
    ...
    >
      ...
      {/* Description – Fix 2: flex-grow damit der Content-Bereich wächst */}
      ...
      {/* CTA – Fix 2: mt-auto damit Button immer unten ist */}

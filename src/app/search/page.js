/**
 * FÖRDERLY – SSR Such-Seite
 * /search?q=...&bundesland=BY&page=2
 *
 * - Server-Side Rendering (keine ISR)
 * - URL-basierte Filter (Query-Params persistent)
 * - Echte Pagination als Seiten
 * - Query Logging
 */
...
export default async function SearchPage({ searchParams }) {
  ...
  const filters = {
    ...
    datenqualitaet: sp?.datenqualitaet || '',
  };
...
function SearchResultCard({ programme, index }) {
  ...
  return (
    <Link
      ...
      className="card p-4 sm:p-5 block animate-fade-up"
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2.5">
        <div className="flex items-center gap-2">
          ...
          <DataQualityBadge quality={programme.datenqualitaet} small />
        </div>
        {hasVolumen && (
